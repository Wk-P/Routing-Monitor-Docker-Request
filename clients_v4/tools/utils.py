...
def write_json_file(filepath: Path, filename: str, data: typing.Dict[typing.AnyStr, typing.Any] | typing.List[typing.Dict[typing.AnyStr, typing.Any]], mode: str):
    if not filepath.exists():
        filepath.mkdir(parents=True, exist_ok=True)
    ...
    if isinstance(data, dict):
        ...
        # 可选：检查列表中的所有元素是否都是 dict
        if all(isinstance(item, dict) for item in data):
            with open(full_path, mode, encoding="utf-8") as json_file:
                json.dump(data, json_file, indent=4, default=custom_serializer)
            return full_path
        else:
            print("Not all dict element")
            return None
    else:
        return None
