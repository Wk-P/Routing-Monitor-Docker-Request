...
def hs_scheduler(cpu_usage_stats):
    ...
    elif hs_down_check(cpu_usage_stats, 20):
    ...
    with ThreadPoolExecutor(max_workers = count + 1) as pool:
        ...
        try:
            while True:
                ...
                for node in swarm_nodes:
                    f = pool.submit(get_cpu_usage(cpu_usage_stats, node.attrs['Status']['Addr'], node))
                    futures.append(f)
                ...
                f = pool.submit(hs_scheduler(cpu_usage_stats))
